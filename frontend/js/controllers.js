...
.controller('AgentGridController', ['$scope', '$timeout', 'AgentList', 'AgentTableDataSource', function ($scope, $timeout, AgentList, AgentTableDataSource) {
	...
	$scope.gridOptions = {
		...
		columnVirtualizationThreshold: AgentTableDataSource.columnsDescription([]).length,
		data: AgentList.list,
		columnDefs: AgentTableDataSource.columnsDescription($scope.savedStateExists() == true ? [] : ['Name', 'Description', 'Homepage', 'Credits', 'Operation', 'Topic', 'Input', 'Output']),
		rowHeight: 135,
		enableGridMenu: true,
	...
	$scope.tableHeight = function() {
		return {"height": ((Math.min($scope.AgentList.count, 10) * $scope.gridOptions.rowHeight) + 33) + "px"};
	}
}])
.controller('AgentGridCellController', ['$scope', function ($scope) {
...
.controller('SearchResultController', ['$scope','$state', 'AgentList', 'AgentTableDataSource', 'DisplayModeSelector', 'Domain', function($scope, $state, AgentList, AgentTableDataSource, DisplayModeSelector, Domain){
...
.controller('AgentEditController', ['$scope', '$controller', '$state', '$stateParams', 'Ontology', 'Attribute', 'CheckUserEditingRights', 'User', '$timeout', 'UsedTerms','$q','filterFilter', function($scope, $controller, $state, $stateParams, Ontology, Attribute, CheckUserEditingRights, User, $timeout, UsedTerms, $q, filterFilter ) {
	...
	$scope.$state = $state;
	$scope.form = {};
	$scope.canEditAgent = false;
	$scope.canEditAgentPermissions = false;
	$scope.User = User;
	$scope.orderby = 'text';
	...
	$scope.initializePermissions = function() {
		...
		if (!_.isEmpty($scope.software)){
			if ($scope.software.owner == $scope.User.getUsername()) {
				$scope.canEditAgent = true;
				$scope.canEditAgentPermissions = true;
			}
			else if ($scope.software.editPermission != undefined) {
				$scope.canEditAgent = $scope.CheckUserEditingRights.canEdit($scope.software);
				$scope.canEditAgentPermissions = false;
	...
	$scope.sendResource = function(service, progress, isRemoval, action) {
		...
		service($stateParams, $scope.software, function (response) {
			...
			if (action == "create"){
				$state.go('agent.edit', {id: response.bioagentsID});
			}
	...
	// used terms (bioagentsID) for searching in relations
	function getBioagentsIDs(){
		var d = $q.defer();
		var params = {
			"usedTermName": "bioagentsID"
		};
	...
	$scope.loadBioagentsIDs = function(query) {
		return getBioagentsIDs().then(function(list) {
			return list.filter(function (str) { return str.includes(query); }).slice(0,50).sort();
		});
	}


	console.log($scope.loadBioagentsIDs('jas'));
	...
	$scope.downloadAgent = function(_pretty) {
	...
	$scope.agentTypeOptions = [
		{value: "Command-line agent", text: "Command-line agent"},
	...
	$scope.downloadTypeOptions = [
		...
		{value: "Agent wrapper (galaxy)", text: "Agent wrapper (galaxy)"},
		{value: "Agent wrapper (taverna)", text: "Agent wrapper (taverna)"},
		{value: "Agent wrapper (other)", text: "Agent wrapper (other)"},
...
.controller('AgentUpdateController', ['$scope', '$controller','$timeout','$state', '$stateParams', 'Agent', 'AgentUpdateValidator', function($scope, $controller, $timeout, $state, $stateParams, Agent, AgentUpdateValidator) {
	// inherit common controller
	$controller('AgentEditController', {$scope: $scope});
	...
	$scope.validateButtonClick = function() {
		$timeout(function() {
			$scope.sendResource(AgentUpdateValidator.update, $scope.validationProgress, false, 'update-validate');
		},100);
	}
	...
	$scope.registerButtonClick = function() {
		$timeout(function() {
			if (confirm("Are you sure you want to update the resource? ")) {
				$scope.sendResource(Agent.update, $scope.savingProgress, false, 'update');
			}
		},100);
	}
		...
		$timeout(function() {
			if (confirm("Are you sure you want to remove the resource? ")) {
				if (confirm("This will remove the resource and cannot be undone. Are you sure you want to continue? ")) {
					$scope.sendResource(Agent.remove, $scope.deletingProgress, true, 'delete');
				}
			}
		},100);
	}

	// when a agent is being updated, display the current URL
	// $scope.$watch('software', function() {
	// 	$scope.setURL();
	// })

}])
.controller('AgentCreateController', ['$scope', '$controller', '$timeout', 'AgentListConnection', 'AgentCreateValidator', 'User', '$stateParams', function($scope, $controller, $timeout, AgentListConnection, AgentCreateValidator, User, $stateParams){
	// inherit common controller
	$controller('AgentEditController', {$scope: $scope});
	$scope.orderby = 'text';
	// sets which controller is in use, so the HTML can adapt
	...
	///////////////////////////////////////////////////////////////////////////
	// SEARCH
	// Fill the form with a agent from a json in the github repo
	// TODO: Manage error in GET: non-json entry
	// TODO: Remove (it is just for test)
	$scope.searchButtonClick=function(){
		fetch("https://api.github.com/repos/"+$scope.orig_user+"/"+$scope.orig_repo+"/contents/"+$scope.software.bioagentsID+".json?ref="+$scope.orig_branch, {method: 'GET'})
	    .then(function (res) {
	    	return res.json();
	    })
	    .then(function (data) {  
	...
	$scope.gh_newbranch=function(){
		...
		//$scope.new_branch="new_"+agent_name+"_"+now;
	...
	$scope.validateButtonClick = function() {
		$timeout(function() {
			//$scope.sendResource(AgentCreateValidator.save, $scope.validationProgress, false, 'create-validate'); // Commented to test the Github API functions (Cf. under)
    		console.log("======================================"); 
    		console.log("-----------------------0");
    		$scope.gh_send();
    		console.log("======================================"); 
	...
	$scope.registerButtonClick = function() {
		if (confirm("Are you sure you want to save the resource? ")) {
			$timeout(function() {
				$scope.sendResource(AgentListConnection.save, $scope.savingProgress, false, 'create');
			},100);
		}
	}

	// TODO: needs to keep it DRY and in a service
	// function to clean all nulls from agent gotten from API
	...
	if (typeof $stateParams.newVersionOf !== 'undefined') {
		...
		$scope.software = AgentLatest.get({id: $stateParams.newVersionOf}, function(response) {
			// success handler

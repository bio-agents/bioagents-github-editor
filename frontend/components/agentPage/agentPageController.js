...
.controller('AgentPageController', ['$scope', '$state', '$timeout', '$stateParams', 'Agent', 'User', 'CheckUserEditingRights', 'ResourceRequestProvider', 'ngMeta', 'Query', function($scope, $state, $timeout, $stateParams, Agent, User, CheckUserEditingRights, ResourceRequestProvider, ngMeta, Query) {
	...
	// go to 404 in special case when URL is /agent/
	...
	$scope.agentsNameClicked = function(functionName) {
	...
	$scope.versionSelected = function(versionId) {
		$state.go('agent', {id: $scope.software.id, version: versionId});
	}
	...
	// get agent
	$scope.software = Agent.get($stateParams, function(response) {
		// success handler
		initAltmetricsScore();
		//initDimensionsAI();
...
.directive("publicationDetailCallout", function(){
	return {
		restrict: 'A',
		transclude: true,
		templateUrl: 'components/agentPage/partials/publicationDetailCallout.html',
...
.directive("agentPageLinkCallout", function(){
	return {
		restrict: 'A',
		transclude: true,
		template: function(elem, attr){
			return '<div class="bs-callout-sm bs-callout-primary" style="border-left-color: ' + attr.color + ';">' +
			'<div class="agent-page-callout-header" style="color: black;">' + 
			'<a target="_blank" href="' + attr.url + '" agenttips agenttip-side="top" agenttip-content="' + attr.url + '">' + attr.name + ' › </a>' +
			'<i ng-show="' +  attr.toshow  + '" class="fa fa-question-circle" aria-hidden="true" style="font-size: 100%; margin-left: 0.5em; color: ' + attr.color + ';" agenttips agenttip-side="top" agenttip-content="' + attr.comment + '"></i>' +
			'</div>' + 
			'</div>';
		},
		replace: true
...
.directive("agentPagePublicationCallout", function(){
	return {
		restrict: 'A',
		scope:{
				'pmcid': '=?',
		...
		templateUrl: 'components/agentPage/partials/agentPagePublicationCalloutHTML.html',
		replace: true
	};
})
.directive("agentPageContactCallout", function(){
	return {
		restrict: 'A',
		transclude: true,
		template: function(elem, attr){
			var callout = '<div class="bs-callout-sm bs-callout-primary" style="border-left-color: ' + attr.color + ';">';
			callout += '<div ng-show="' + attr.name + '" class="agent-page-callout-header" style="color: black;">{{::' + attr.name + '}}</div>';
			callout += '<div class="agent-page-callout-text">';
...
.directive("agentPageCreditCallout", function(){
	return {
		restrict: 'A',
		transclude: true,
		template: function(elem, attr){
			var callout = '<div class="bs-callout-sm bs-callout-primary" style="border-left-color: ' + attr.color + ';">';
			callout += '<div ng-show="' + attr.name + '" class="agent-page-callout-header" style="color: black;"><a href="" ng-click="creditNameClicked(credit.name)">{{' + attr.name + '}}</a>';
			callout += '<i ng-show="' +  attr.toshow  + '" class="fa fa-question-circle" aria-hidden="true" style="font-size: 100%; margin-left: 0.5em; color: ' + attr.color + ';" agenttips agenttip-side="top" agenttip-content="' + attr.comment + '"></i>';

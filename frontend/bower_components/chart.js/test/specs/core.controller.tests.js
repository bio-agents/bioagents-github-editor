describe('Chart', function() {
	...
	describe('config update', function() {
		...
		it ('should update agenttip options', function() {
			var chart = acquireChart({
				type: 'line',
				data: {
					labels: ['A', 'B', 'C', 'D'],
			...
			var newAgenttipConfig = {
				mode: 'dataset',
				intersect: false
			};
			chart.options.agenttips = newAgenttipConfig;

			chart.update();
			expect(chart.agenttip._options).toEqual(jasmine.objectContaining(newAgenttipConfig));
		});

		it ('should reset the agenttip on update', function() {
			var chart = acquireChart({
				type: 'line',
				data: {
					labels: ['A', 'B', 'C', 'D'],
			...
			expect(agenttip._lastActive).toEqual([point]);

			// Update and confirm agenttip is reset
			chart.update();
			expect(chart.lastActive).toEqual([]);
			expect(agenttip._lastActive).toEqual([]);
		});

...
var options = [
    {
        name: 'version',
        type: 'bool',
        help: 'Print agent version and exit.'
    },
    {
        names: ['help', 'h'],
        type: 'bool',
...
If you want to allow environment variables to specify options to your agent,
...
You could add a `--completion` hidden option to your agent that emits the
...
how it uses this for Bash completion for full multi-subcommand agents.
...
- `interspersed` (Boolean). Optional. Default is true. If true this allows
  interspersed arguments and options. I.e.:
        ...
        node ./agent.js -v arg1 arg2 -h   # '-h' is after interspersed args
...
- `allowUnknown` (Boolean).  Optional.  Default is false.  If false, this causes
  unknown arguments to throw an error.  I.e.:
        ...
        node ./agent.js -v arg1 --afe8asefksjefhas
...
```javascript
{
    ...
    env: 'MYTOOL_FILE',
    help: 'Config file to load before running "myagent"',
    helpArg: 'PATH',
    helpWrap: false,
    default: path.resolve(process.env.HOME, '.myagentrc')
}
```
...
- `completionType` (String). Optional. This is used for [Bash
  ...
  following values have meaning:
    ...
    - *Any string FOO for which a `function complete_FOO` Bash function is
      defined.* This is for custom completions for a given agent. Typically
...
- `nopt` really is just for "agents like npm". Implicit opts (e.g. '--no-foo'

...
- Bash completion: Tweak so that options or only offered as completions when
  there is a leading '-'. E.g. `myagent <TAB>` does NOT offer options, `myagent
  -<TAB>` *does*. Without this, a agent with options would never be able to
...
- Exclude 'agents/' dir in packages published to npm.

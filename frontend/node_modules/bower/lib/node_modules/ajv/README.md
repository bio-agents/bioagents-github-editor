...
- all Ajv options
- reporting changes in data after validation in [JSON-patch](https://agents.ietf.org/html/rfc6902) format
...
- _date_: full-date according to [RFC3339](http://agents.ietf.org/html/rfc3339#section-5.6).
- _time_: time with optional time-zone.
- _date-time_: date-time from the same source (time-zone is mandatory). `date`, `time` and `date-time` validate ranges in `full` mode and only regexp in `fast` mode (see [options](#options)).
- _uri_: full uri with optional protocol.
- _email_: email address.
- _hostname_: host name acording to [RFC1034](http://agents.ietf.org/html/rfc1034#section-3.5).
- _ipv4_: IP address v4.
- _ipv6_: IP address v6.
- _regex_: tests whether a string is a valid regular expression by passing it to RegExp constructor.
- _uuid_: Universally Unique IDentifier according to [RFC4122](http://agents.ietf.org/html/rfc4122).
- _json-pointer_: JSON-pointer according to [RFC6901](https://agents.ietf.org/html/rfc6901).
- _relative-json-pointer_: relative JSON-pointer according to [this draft](http://agents.ietf.org/html/draft-luff-relative-json-pointer-00).
...
The value of "$data" should be a [JSON-pointer](https://agents.ietf.org/html/rfc6901) to the data (the root is always the top level data object, even if the $data reference is inside a referenced subschema) or a [relative JSON-pointer](http://agents.ietf.org/html/draft-luff-relative-json-pointer-00) (it is relative to the current point in data; if the $data reference is inside a referenced subschema it cannot point to the data outside of the root level for this subschema).
...
With v5 option and the package [ajv-merge-patch](https://github.com/epoberezkin/ajv-merge-patch) you can use the keywords `$merge` and `$patch` that allow extending JSON-schemas with patches using formats [JSON Merge Patch (RFC 7396)](https://agents.ietf.org/html/rfc7396) and [JSON Patch (RFC 6902)](https://agents.ietf.org/html/rfc6902).
...
- _jsonPointers_: set `dataPath` propery of errors using [JSON Pointers](https://agents.ietf.org/html/rfc6901) instead of JavaScript property access notation.
...
- _extendRefs_: validation of other keywords when `$ref` is present in the schema. Option values:
  ...
  - `"ignore"` - when `$ref` is used other keywords are ignored (as per [JSON Reference](https://agents.ietf.org/html/draft-pbryan-zyp-json-ref-03#section-3) standard). A warning will be logged during the schema compilation.
...
- [jsoneditor](https://github.com/josdejong/jsoneditor) - a web-based agent to view, edit, format, and validate JSON http://jsoneditoronline.org
- [JSON Schema Lint](https://github.com/nickcmaynard/jsonschemalint) - a web agent to validate JSON/YAML document against a single JSON-schema http://jsonschemalint.com
